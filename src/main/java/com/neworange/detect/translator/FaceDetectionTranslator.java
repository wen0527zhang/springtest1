...
public class FaceDetectionTranslator implements Translator<Image, DetectedObjects> {
    ...
    /**
     * 相当于numpy.ndarray，可以通过getShape()方法获取其shape
     * NDManager:NDArray的管理类，全局new一个就行了，需要用该类对象创建NDArray 全局应只创建一个
    ...
    public NDList processInput(TranslatorContext ctx, Image input) {
        ...
        //对数组进行翻转可以使用[..., ::-1]，但java中不行，但可以利用flip函数实现
    ...
    public DetectedObjects processOutput(TranslatorContext ctx, NDList list) {
        ...
        prob =NDArrays.stack(new NDList(prob.argMax(1).toType(DataType.FLOAT32, false),
                                prob.max(new int[]{1})));
        ...
        NDArray bbXY = boundingBoxes.get(":, :2").mul(scaleXY).mul(boxRecover.get(":, 2:"))
                        .add(boxRecover.get(":, :2")).sub(bbWH.mul(0.5f));
