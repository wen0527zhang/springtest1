...
public class OCRRecTranslator implements Translator<Image, WordBlock> {
    ...
    public OCRRecTranslator(Map<String, ?> arguments) {
        use_space_char = arguments.containsKey("use_space_char")
    ...
    public void prepare(TranslatorContext ctx) throws IOException {
        try {
            JarFileUtils.copyFileFromJar(File.separator+"onnx"+File.separator+"models"+File.separator+"ppocr_keys_v1.txt", PathConstants.ONNX, null, false, true);
        } catch (IOException e) {
    ...
    @Override
    public WordBlock processOutput(TranslatorContext ctx, NDList list) throws IOException {
