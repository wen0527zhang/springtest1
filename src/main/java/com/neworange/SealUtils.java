...
public class SealUtils {
   ...
   // private static final String FOOT_TXT = "金富源";// 底部文字
    ...
    public static void main(String[] args) {
        try {
            String name = "我的印章就是大家思考就开始健康的就是";
            String footName="电子签约专用章";
            byte[] img = SealUtils.genSealData(name,footName);
            byte2File(img, "a", name + ".png");
        } catch (IOException e) {
     ...
     * @return
     */
    public static BufferedImage genSealImage(String companyName,String footName) {
        ...
        drawFootText(g2d, circleRadius,footName);
     ...
     * @throws IOException
     */
    public static byte[] genSealData(String companyName,String footName) throws IOException {
        BufferedImage image = genSealImage(companyName,footName);
     ...
     * @param circleRadius
     */
    private static void drawFootText(Graphics2D g2d, int circleRadius,String footName) {
        Font f;
        FontRenderContext context;
        Rectangle2D rectangle;
        f = new Font(FONT_FAMILY, Font.BOLD, FOOT_FONT_SIZE);
        context = g2d.getFontRenderContext();
        rectangle = f.getStringBounds(footName, context);
        int a = 10 * CANVAS_SIZE / 350; // 底部文字距中心的距离，这是调出来的
        g2d.setFont(f);
        g2d.drawString(footName, (float) (circleRadius + a - rectangle.getCenterX()), (float) (circleRadius * 1.5 + a - rectangle.getCenterY()));
    }
