...
public class FileService {
     ...
     */
    private String downloadPath = System.getProperty("user.dir") + "/download/";
    ...
    public void upload(HttpServletRequest request, HttpServletResponse response) throws Exception {
        // 获取ServletFileUpload
        JakartaServletFileUpload servletFileUpload = getServletFileUpload();
     ...
     * 获取ServletFileUpload
     */
    private JakartaServletFileUpload getServletFileUpload() {
        // 设置缓冲区大小，先读到内存里在从内存写
        ...
        for (FileItem item : items) {
            if (item.isFormField()) {
                ...
                if ("chunk".equals(item.getFieldName())) {
                    uploadFileInfo.setCurrentChunk(Integer.parseInt(item.getString()));
                }
                if ("chunks".equals(item.getFieldName())) {
                    uploadFileInfo.setChunks(Integer.parseInt(item.getString()));
                }
                if ("name".equals(item.getFieldName())) {
                    uploadFileInfo.setFileName(item.getString());
                }
            }
        }
